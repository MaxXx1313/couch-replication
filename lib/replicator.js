...
class Replicator extends EventEmitter {
  ...
  replicate(source, target, opts) {
    ...
    let progress = typeof opts.progress !== "undefined" ? opts.progress : DEFAULT_PROGRESS_TIMEOUT;
    if(progress < MIN_TIMEOUT) progress = MIN_TIMEOUT;
  ...
  _getProgress(source, target){
    this.replicationList().then(list=>{
      console.log(list);
    });
  }
