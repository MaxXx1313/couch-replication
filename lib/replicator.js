...
function getByType(dbUrl, type){
  // do it
  return _getByType_couch1x(dbUrl, type)
  .catch(e=>{
    if( e.statusCode !== 403
      || e.reason !== 'Temporary views are not supported in CouchDB'){

      throw e;
    }
    return _getByType_couch2x(dbUrl, type);
  });
}


// couch 1.6
function _getByType_couch1x(dbUrl, type){
  let parsed = parseDbUrl(dbUrl);
  return nano(parsed.host).request({
    db: parsed.db,
    doc:'_temp_view',
...
// couch 2.0
function _getByType_couch2x(dbUrl, type){
  let parsed = parseDbUrl(dbUrl);
  return nano(parsed.host).request({
      db: parsed.db,
      doc:'_find',
      body:{"selector":{"type":type}},
