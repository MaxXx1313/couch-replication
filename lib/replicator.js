...
class Replicator extends EventEmitter {
  ...
  _copyUsers(dbFrom, dbTo){
    ...
    self.emit('opStart', 'Transfer users' );
    return Replicator.getDbUsers(dbFrom)
      .then(userData=>{
        ...
        .then(()=>{
          Replicator.setDbUsers(dbTo, userData);
        });
      })
      .then(()=>{
        self.emit('opEnd', 'Done');
  ...
  _copyUser(userId, from, to){
    return Replicator.getUser(from, userId)
      .then(user=>{
        ...
        return Replicator.setUser(to, user._id, user);
      });
  }
...
function setUser(host, userId, userData){
  return nano(host).request({
      db: '_users',
      doc: 'org.couchdb.user:' + userId,
      body: userData,
